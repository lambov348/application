...
## Планировщик (автозакрытие смен в 23:59)

Локально в отдельном окне терминала: `php artisan schedule:work`.
На сервере — одна строка в cron: `* * * * * cd /путь/к/проекту && php artisan schedule:run`.

## Пуш-уведомления

1. Один раз создать ключи: `php artisan webpush:vapid --write` (записываются в `.env`).
2. Отправка идёт в фоне — в отдельном окне терминала: `php artisan queue:work`.
   На сервере этот процесс держит запущенным Supervisor.
3. Пуши работают только по HTTPS (или на `localhost`). На iPhone — только из приложения,
   установленного на экран «Домой» (iOS 16.4+).
